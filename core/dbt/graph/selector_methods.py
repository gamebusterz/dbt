...
class SelectorMethod(metaclass=abc.ABCMeta):
    ...
    ) -> Iterator[Tuple[str, NonSourceNode]]:
    ...
    def source_nodes(
        self,
        included_nodes: Set[str]
    ) -> Iterator[Tuple[str, ParsedSourceDefinition]]:
    ...
    def all_nodes(
        self,
        included_nodes: Set[str]
    ) -> Iterator[Tuple[str, Union[ParsedSourceDefinition, NonSourceNode]]]:
        yield from chain(self.parsed_nodes(included_nodes),
                         self.source_nodes(included_nodes))

    @abc.abstractmethod
    def search(self, included_nodes: Set[str], selector: str) -> Iterator[str]:
...
class TagSelectorMethod(SelectorMethod):
    def search(self, included_nodes, selector) -> Iterator[str]:
